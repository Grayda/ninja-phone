...
function Device() {
 ...
 this.serialPort.on('data', function(data) { // When data is received from the modem
	 ...
	 res = parseData(data.toString().trim()); // the parseData function takes our raw data, finds out what's in it, and returns the relevant information (e.g. RING status or caller ID)
...
function parseData(data) { // Parses the incoming modem instructions (e.g. RING, NMBR etc.);
	...
	res = {};
	var nmbr;
	var nmbrpos = data.indexOf("NMBR"); // See if we've got NMBR in the data (which indicates our Caller ID data has come through)
	...
	if(data == "RING") {
		...
		res.data = "NMBR";
		res.nmbr = data.substring(nmbrpos + 7);
		console.log("Caller ID info found. " + res.nmbr + " was the number extracted");
	} else {
		res.data = ""; // Err, need to do something about this, as a modem command other than caller ID info or RING could cause a crash or something?
	}
